...
void inputText(text *newText, char file_name[])
{
	...
	for (i = 0; i <= ROWS; i++)
	{
		...
		if (baris == ROWS)
		{ // Jika baris sudah berada diakhir maka jalankan modul save
			printf("\n");
		...
		for (j = 0; j <= COLUMNS; j++)
		{
			...
			{ // Pengkondisian jika karakter input atau sudah melebihi batas kolom maka hentikan loop
				break;
			}
			else if (temp == COPY || temp == PASTE || temp == UNDO || temp == REDO || temp == BACKSPACE)
			{ // Pengkondisian jika input merupakan shortcut maka variabel pencacah tidak menambah nilainya
				j--;
			}
...
{ // Memiliki karakteristik yang sama dengan modul inputText hanya berbeda pada parameter
